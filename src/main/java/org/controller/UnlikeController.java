...
public class UnlikeController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result=lcSer.unLikePost(postid, userID);
		if(result!=0) {
			// get count of like
            ...
            int v=lk.checkLike(postid,userID);
			 if(v>0){
				 out.println("<a id='liked' onclick='unlikefun("+postid+")'> <i class='fa-solid fa-heart'></i>&nbsp"+likeCount+"</a>");
             }else{
				out.println("<a id='like'  onclick='likefun("+postid+")'> <i class='fa-solid fa-heart'></i>&nbsp"+likeCount+"</a>");
			 }
